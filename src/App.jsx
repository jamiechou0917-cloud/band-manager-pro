...
const App = () => {
  ...
  // === 🚀 修改：加入 Google Calendar API 授權與 Token 儲存 ===
  const handleLogin = async () => {
    try { 
      ...
      // 取得 Google Access Token 並存入 localStorage 供後續加入日曆使用
...
const DashboardView = ({ members = [], generalData = {}, alcoholCount = 0, db, role = {}, user }) => {
  ...
  const groupedLogs = useMemo(() => {
    return (generalData.practices || []).reduce((acc, log) => {
      if (!log.date) return acc;
      const month = log.date.substring(0, 7); // 擷取 "YYYY-MM"
      if (!acc[month]) acc[month] = [];
      acc[month].push(log);
      return acc;
    }, {});
  }, [generalData.practices]);
  ...
  // === ✅ 新增：串接 Google Calendar API 加入行事曆 ===
  const handleAddEventToCalendarAPI = async (log, e) => {
    e.preventDefault(); 
    const token = localStorage.getItem('googleCalendarToken');
    
    if (!token) {
      alert('請先登出，然後重新點擊 Google 登入以授權日曆權限！');
      return;
    }

    // 格式化時間，確保符合 RFC3339 格式 (加上秒數 :00)
    const formatTime = (dateStr) => {
      if(dateStr.length === 16) return `${dateStr}:00+08:00`; 
      return dateStr;
    };

    const startDate = formatTime(log.date);
    let endDate;
    if (log.endTime) {
      endDate = formatTime(log.endTime);
    } else {
      const d = new Date(log.date);
      d.setHours(d.getHours() + 2);
      endDate = formatTime(d.toISOString().slice(0, 16)); 
    }

    const event = {
      summary: log.title || '不開玩笑練團',
      location: log.location || '圓頭音樂',
      description: `${log.targetSongs ? '🎵 預計曲目: ' + log.targetSongs : ''}${log.memo ? '\n📝 備註: ' + log.memo : ''}`,
      start: { dateTime: startDate, timeZone: 'Asia/Taipei' },
      end: { dateTime: endDate, timeZone: 'Asia/Taipei' },
    };
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
      ...
      {/* ✅ 修改：練團日誌手風琴列表 (還原莫蘭迪設計) */}
      <div className="bg-white p-4 rounded-[2rem] border border-[#E0E0D9] shadow-sm">
         <div className="font-bold text-[#725E77] mb-4 flex items-center gap-2 px-2"><Calendar size={18}/> 場次列表</div>
         <div className="space-y-4">
              ...
              .map(([month, monthLogs]) => (
                <div key={month} className="space-y-2">
                  ...
                  >
                    ...
                    {openMonths[month] ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                  </button>
                  
                  {/* 該月日誌清單 */}
                  {openMonths[month] && (
                    <div className="space-y-2 pt-1 pb-2">
                      {monthLogs.sort((a,b) => new Date(b.date) - new Date(a.date)).map((p, idx) => (
                        <div key={idx} className="flex justify-between items-start text-sm p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                           <div className="flex-1">
                               <div className="font-bold text-[#6D6176] text-lg mb-1 tracking-wide">{new Date(p.date).toLocaleDateString()} {p.title}</div>
                               <div className="text-xs text-[#A29A8C] font-bold mb-1.5 flex items-center gap-1.5">
                                  <span className="text-[#DBCBD8] text-[10px]">◎</span> 
                               ...
                               {p.memo && <div className="text-[10px] text-[#77ABC0] bg-[#77ABC0]/10 px-2 py-1 rounded-lg w-fit mt-1 flex items-center gap-1"><StickyNote size={10}/> {p.memo}</div>}
                           </div>
                           
                           {/* ✅ 呼叫 API 的加入行事曆按鈕 */}
                           <button 
                             onClick={(e) => handleAddEventToCalendarAPI(p, e)} 
                             className="w-10 h-10 bg-[#FDFBF7] border border-[#F2EFE8] rounded-full flex items-center justify-center text-[#DBCBD8] hover:text-[#B59EAF] hover:bg-white transition shadow-sm shrink-0 ml-2" 
                             title="加入 Google 日曆"
                           >
            ...
            {Object.keys(groupedLogs).length === 0 && <div className="text-xs text-slate-400 text-center py-4">尚無場次安排</div>}
         </div>
      </div>
